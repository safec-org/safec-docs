...
| 언어 | macOS (M1 Pro) | WSL2 (7800X3D) | Windows (7800X3D) |
|---|---|---|---|
| SafeC (아레나) | **0.090s (가장 빠름)** | 0.154s | 0.112s |
...
|---|---|---|---|---|
| SafeC | **35832 (가장 빠름)** (1/5ms) | 17497 (3/4ms) | 2043 (9.1/21.2ms) | 7.2 MB |
...
::: tip `http_serve_reactor` 대 기존 `http_serve_threaded`
위 SafeC 수치는 `std::http_serve_reactor`를 씁니다 — 고정 크기 블로킹 스레드 풀 모델(`http_serve_threaded`)을 대체하는 논블로킹 이벤트 루프(`std::Reactor`/`std::TaskScheduler`, kqueue/epoll/WSAPoll 기반)입니다. 전환 결과: **macOS 28305→35832 req/s (+27%)**, **WSL2 16102→17497 req/s (+9%)**, 둘 다 `numThreads=1`에서 — 리액터 스레드를 늘릴수록 두 플랫폼 모두 처리량이 오히려 나빠졌습니다(같은 리스닝 소켓에서 각 스레드가 독자적으로 `accept()`를 호출하면, 이렇게 가벼운 핸들러에서는 병렬화보다 경합이 더 커집니다). **Windows는 1135→2043 req/s**(동일 규모 기준)로 개선됐지만, macOS/WSL2에서는 본 적 없는 수준으로 실행마다 편차가 컸습니다 — `WSAPoll`은 kqueue/epoll과 달리 커널 측 영속 등록이 없어서, poll을 호출할 때마다 감시 대상 집합을 처음부터 다시 만들고 다시 훑습니다. 이는 이번 작업으로는 없애지 못한 실제 아키텍처상의 한계입니다. 이 작업 과정에서 잠재해 있던 버그 다섯 개를 찾아 모두 `std/`에서 고쳤습니다: Apple Silicon에서 논블로킹 모드를 조용히 깨뜨리던 `fcntl` ABI 불일치, macOS에서 잘못 하드코딩되어 있던 `EAGAIN`/`EWOULDBLOCK` 값, Windows 소켓에서 "블로킹될 것"을 이식 가능한 방식으로 감지할 수단이 없던 문제, 오래된 Windows 오류 코드가 acceptor를 영원히 멈춰 세우게 만들던 `tcp_accept_nb`의 누락된 실패 검사, 그리고 중복 정의된 `struct WSAData_`.
:::
