...
int main() {
    ...
    unsafe {
        ...
        void* bufX = std::mps_upload_bf16_persistent((const float*)xbuf, BATCH * IN_DIM);
        void* bufW1 = std::mps_upload_bf16_persistent((const float*)w1buf, IN_DIM * HIDDEN);
        void* bufW2 = std::mps_upload_bf16_persistent((const float*)w2buf, HIDDEN * OUT_DIM);

        unsigned short* tHalf = (unsigned short*)malloc(sizeof(unsigned short) * BATCH * OUT_DIM);
        std::f32_to_bf16_bulk((const float*)tbuf, tHalf, BATCH * OUT_DIM);
        ...
        while (step < STEPS) {
            ...
            std::mps_matmul_bf16_persistent(bufX, bufW1, scratchZ1, BATCH, IN_DIM, HIDDEN);
            void* bufZ1 = std::mps_batch_last_output_buffer();
            std::mps_relu_bf16_chained(bufZ1, scratchH, BATCH * HIDDEN);
            void* bufH = std::mps_batch_last_output_buffer();
            std::mps_matmul_bf16_persistent(bufH, bufW2, scratchY, BATCH, HIDDEN, OUT_DIM);
            void* bufY = std::mps_batch_last_output_buffer();

            std::mps_sub_bf16_chained(bufY, (const unsigned short*)tHalf, scratchDiff, BATCH * OUT_DIM);
            void* bufDiff = std::mps_batch_last_output_buffer();

            std::mps_scale_bf16_chained(bufDiff, 2.0f, scratchDY, BATCH * OUT_DIM);
            void* bufDY = std::mps_batch_last_output_buffer();

            std::mps_matmul_atb_bf16_chained_ab(bufH, bufDY, scratchDW2, BATCH, HIDDEN, OUT_DIM);
            void* bufDW2 = std::mps_batch_last_output_buffer();

            std::mps_matmul_abt_bf16_persistent(bufDY, bufW2, scratchDH, BATCH, OUT_DIM, HIDDEN);
            void* bufDH = std::mps_batch_last_output_buffer();

            std::mps_relu_backward_bf16_chained_ab(bufZ1, bufDH, scratchDZ1, BATCH * HIDDEN);
            void* bufDZ1 = std::mps_batch_last_output_buffer();

            std::mps_matmul_atb_bf16_chained_ab(bufX, bufDZ1, scratchDW1, BATCH, IN_DIM, HIDDEN);
            void* bufDW1 = std::mps_batch_last_output_buffer();

            std::mps_sgd_update_bf16_chained(bufW2, bufDW2, (float)LR, HIDDEN * OUT_DIM);
            std::mps_sgd_update_bf16_chained(bufW1, bufDW1, (float)LR, IN_DIM * HIDDEN);
            ...
            while (j < BATCH * OUT_DIM) {
                float d = std::bf16_to_f32(scratchDiff[j]);
